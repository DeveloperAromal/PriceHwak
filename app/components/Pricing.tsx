...
function PricingCard({ name, price, timeframe, features, useClient }: PricingProps & { useClient: boolean }) {
  // Modify pricing based on the useClient flag
  const actualPrice = useClient ? parseFloat(price.slice(1)) * 1.2 : parseFloat(price.slice(1));

  return (
    <div className="w-80 md:w-80 lg:w-80 mb-8">
      <div className={`card_one border-2 border-teal-400 ${
        name === "Standard" ? "h-100 relative top-4 pricing-shad-cyan-500" : "bg-black pricing-shad-cyan-700"
        } flex flex-col items-center py-4 px-4`}>
        <div className="main flex flex-col items-center justify-center mb-8">
          <h4 className="mb-2">{name}</h4>
          <h1 className="text-4xl md:text-5xl lg:text-6xl mb-4">${actualPrice.toFixed(2)}</h1>
          ...
          <ul>
            {features.map((feature, index) => (
              <li key={index} className="flex items-center mb-2">
                <div className={feature.available ? "text-green-500" : "text-red-500"}>
                  {feature.available ? "✅" : "❌"}
                </div>
